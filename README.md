...
Functional programmers are familiar with Option types (or `Maybe`s, in Haskell-speak).  superoption extends the idea of an option type from just `None` and `Some` (in SML-speak) to `None`, `Some`, and `Lots`.  This allows programmers more fine-grained control over their types that can hold any number of values (ours are called "superoption").
